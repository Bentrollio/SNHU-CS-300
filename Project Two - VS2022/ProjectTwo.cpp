/*
 * PROJECT TWO - ABCU Computer Science Course Catalog
 * By Alex Baires
 * 
 * Version: 1.1
 */
...
/**
 * TEST function to double-check the tree is balanced, not implemented in menu!
 * Visits the root, then traverses the left subtree, then the right one.
 *
 * @param node - Current node in tree
 */
...
/*
 **********************************************
 * FILE ERROR-CHECKING AND PARSING OPERATIONS *
 **********************************************
 */
...
/**
 * Takes a line and splits each word into an element inside a vector of strings.
 *
 * @param - A stringstream
 * @return - A vector of strings used to store tokenized words from the line
 */
static vector<string> tokenizeLine(stringstream& stream, vector<string>& lineVector) {
    string token;
    lineVector.clear(); // Prepares vector of tokenized words from line
    while (getline(stream, token, ',')) { // Parses through each word in line
        findBOM(token); // Cleans the token
...
static vector <string> loadDefaultCourseList(const string& filePath) {
    ...
    while (getline(courseData, line)) { // Parses through each line in file
        ...
        tokenizeLine(courseStream, temp); // Generates temporary line vector from stream
        defaultCourses.push_back(temp.at(0)); // First element of temp is the default course in catalog
    }

    temp.clear(); // Freeing up memory after we exit while loop
    courseData.close();
...
static vector<string> verifyDataIntegrity(vector<string>& courseTokens, vector<string>& courseCatalog) {
    ...
    if (courseTokens.size() < 2) { // Verifies that each line has at least 2 elements
        ...
        throw "Above Course Line Incomplete, please update file with at least COURSE ID and COURSE TITLE separated by a comma";
    }
...
static vector<vector<string>> fileParser(const string& filePath) {
    ...
    if (courseData.fail()) { // Error handling in case file does not open
        throw "Double-check course file path.";
    }

    while (getline(courseData, line)) { // Parses through each line in file
        stringstream courseStream(line);
        tokenizeLine(courseStream, tokens);
        verifyDataIntegrity(tokens, courseParameters);
        fileContents.push_back(tokens);
    }

    cout << "File successfully processed." << endl;
    tokens.clear(); // Freeing up memory after we exit loop
    courseData.close();
    return fileContents;
}
...
/*
 **********************************
 * LOADING INFO TO DATA STRUCTURE *
 **********************************
 */
...
/**
 * DEPRECATED LEGACY Function - This was my original implementation, but has been superseded by the
 * loadBalancedTree() function.
 * 
 * Inserts each course object into the BST.
 *
...
/**
 * Compares two Course objects by their course IDs.
 *
 * @param - two course objects.
 * @return - true depending on if the first course objects courseID is less than the second's.
 *
 */
...
/**
 * Sorts a vector in ascending order. It uses the 
 * compareCourseID function to order the Course objects by name. 
 * It will be used to create a balanced BST from the file to
...
/**
 * Uses the sorted vector and gets the middle element to insert as the root.
 * Recursively calls itself so that the lower and higher parts of the course vector are inserted into the tree,
 * creating a balanced BST for an efficient O(log n) runtime.
 *
 * @param - courses - A vector of course objects
 * @param - tree - An empty BST
 * @param - int start and end - the first and last element numbers of the vector
 *
 */
...
/*
 ***************************************
 * PROGRAM OUTPUT - COURSE INFORMATION *
 ***************************************
 */
...
static void mainMenu(vector <Course>& courseVector, BinarySearchTree* tree) {
    ...
    while (userInput != 9) {
        ...
        // Handles an infinite loop if the user accidentally enters a string instead of int (like I did)
        if (cin.fail()) // https://stackoverflow.com/questions/5864540/infinite-loop-with-cin-when-typing-string-while-a-number-is-expected
        {
        ...
        switch (userInput) {
        case 1:
            //loadCoursesIntoTree(courseVector, tree); // ORIGINAL IMPLEMENTATION
        ...
        case 3:
            ...
            // Ensures the user does not need to worry about case
